    ...
    <div className="min-h-screen px-6 md:px-12 py-20 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col gap-8 text-sm leading-relaxed" style={{ color: "var(--color-text-secondary)" }}>
        ...
        <section>
          ...
          <p><strong style={{ color: "var(--color-text)" }}>Agent event data:</strong> Clawnitor captures tool calls, LLM requests, messages, and lifecycle events from your OpenClaw agents via the Clawnitor plugin. This data powers monitoring, alerting, rule evaluation, and anomaly detection.</p>
          ...
          <p className="mt-2"><strong style={{ color: "var(--color-text)" }}>Auto-kill configuration:</strong> For each agent, Clawnitor stores your auto-kill settings (enabled/disabled, violation threshold, detection window). Violation logs tracking rule breaches that trigger auto-kill are maintained in your workspace and are subject to the same retention policy as event data.</p>
        </section>

        <section>
          <h2 className="text-lg font-semibold mb-3" style={{ color: "var(--color-text)" }}>Automatic Secret Redaction</h2>
          <p>Before any event data leaves your agent, the Clawnitor plugin automatically redacts sensitive information using 25+ built-in patterns: API keys, passwords, tokens, SSH keys, AWS credentials, database connection strings, and OAuth tokens. Raw secrets are <strong style={{ color: "var(--color-text)" }}>never transmitted to or stored on Clawnitor servers</strong>. You can add custom redaction patterns in your plugin configuration.</p>
        </section>
        ...
        <section>
          ...
          <p className="mt-2"><strong style={{ color: "var(--color-text)" }}>Clawnitor does not sell, rent, or trade personal data to third parties.</strong></p>
        </section>
        ...
        <section>
          ...
          <ul className="list-disc pl-6 mt-2 flex flex-col gap-1.5">
            ...
            <li><strong style={{ color: "var(--color-text)" }}>OpenAI</strong> — AI fallback provider and feedback categorization. If the primary provider is unavailable, Clawnitor automatically switches to OpenAI. Data is not used for model training.</li>
        ...
        <section>
          ...
          <p className="mt-2"><strong style={{ color: "var(--color-text)" }}>California users (CCPA):</strong> You have the right to know what data is collected, request deletion, and opt out of data sharing. Clawnitor does not sell personal data.</p>
          <p className="mt-2">To exercise these rights, email <a href="mailto:privacy@clawnitor.io" style={{ color: "var(--color-coral)" }}>privacy@clawnitor.io</a>. We respond within 45 days.</p>
        </section>
        ...
        <section>
          ...
          <p className="mt-2">Report security vulnerabilities to <a href="mailto:security@clawnitor.io" style={{ color: "var(--color-coral)" }}>security@clawnitor.io</a>. We will acknowledge reports within 48 hours and provide a resolution timeline within 5 business days.</p>
        </section>

        <section>
          <h2 className="text-lg font-semibold mb-3" style={{ color: "var(--color-text)" }}>Cookies &amp; Tracking</h2>
          <p>Clawnitor does not use cookies for tracking, profiling, or marketing. Cookies are used only for session management (login tokens). No third-party analytics or tracking pixels are loaded.</p>
          <p className="mt-2"><strong style={{ color: "var(--color-text)" }}>Landing page analytics:</strong> Clawnitor collects anonymous page view data on the marketing site (clawnitor.io) including page path, referrer URL, and inferred country (via request headers). This data is stored in our own database, is not linked to user accounts, and is used solely to understand traffic patterns. No browsing history, device fingerprints, or personal identifiers are collected. Analytics data is retained for 30 days and is not shared with third parties.</p>
        </section>
        ...
        <section>
          ...
          <p>Questions about privacy? Email <a href="mailto:privacy@clawnitor.io" style={{ color: "var(--color-coral)" }}>privacy@clawnitor.io</a>.</p>
        </section>
      </div>
    </div>
  );

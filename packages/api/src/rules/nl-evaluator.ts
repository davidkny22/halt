...
import { evaluate } from "../ai/haiku-client.js";
...
export async function evaluateNLRule(
  ...
  const userPrompt = `Rule to evaluate: "${ruleText}"
...
Does this batch of events violate the rule?`;
  ...
  if (!response) {
    return { triggered: false, confidence: 0, explanation: "Haiku unavailable" };
  }
  ...
  try {
    ...
    return { triggered: false, confidence: 0, explanation: "Failed to parse Haiku response" };
  }
}
